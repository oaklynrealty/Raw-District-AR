      ...
      <div class="shell contact-layout">
        <div class="form-intro compact-intro">
          ...
          <div class="mini-compliance">
            ${[t("basic_enquiry_only"), t("subject_to_confirmation"), t("no_guaranteed_returns")]
              .filter(Boolean)
        ...
        <div class="form-panel">
          <form id="landingLeadForm" novalidate>
            ...
            ${project.form.privacyText ? `<p class="disclaimer">${renderBrandText(project.form.privacyText)}</p>` : ""}
            ...
            <div class="btn-outer">
              ...
              <button id="landingSubmitBtn" class="btn btn-primary" type="submit" style="position:relative; z-index:2;">${escapeHtml(t("form_submit"))}</button>
            </div>
            ${project.form.whatsappAlternativeLabel ? `<div class="form-whatsapp-alt">
              <span>${escapeHtml(project.form.whatsappAlternativeLabel)}</span>
              ${renderWhatsAppLink({ className: "btn btn-whatsapp", label: project.form.whatsappButtonLabel || t("hero_whatsapp_cta"), location: "form_whatsapp_alternative" })}
            </div>` : ""}
...
if (publicRoutePath) {
  await writeGeneratedFile(`${publicRoutePath}/index.html`, renderLandingPage(landingProject));
}
...
if (internalEnglishThankYouPath) {
  await writeGeneratedFile(
    `${internalEnglishThankYouPath}/index.html`,
    renderThankYouPage({
      currentProject: landingProject,
      canonical: landingProject.thankYouPageUrl,
...
if (internalArabicThankYouPath) {
  await writeGeneratedFile(
    `${internalArabicThankYouPath}/index.html`,
    renderThankYouPage({
